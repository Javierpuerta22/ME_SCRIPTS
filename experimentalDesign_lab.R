...
# Author(s):     Karina Gibert i Sergi Ramírez
#                        IDEAI (c)
...
# Realizamos un random de lending ya que tenemos mucha información
## set.seed(06071994)
...
lending <- get(load(paste0(path, "lendingClub.RData")))

# Examine the variables with glimpse()
glimpse(lending)

lending %>% summarize(median(loan_amnt), 
                          mean(int_rate))
...
model <- lm(weight_loss ~ gender * exercise, data = data) 
