...
plot(hc)
hc$height # para mirar cuántos componentes hay (hay 997)
barplot(hc$height[1:997],main="Aggregated distance at each iteration")
...
dcon <- dd[,c(2,8,11,12,13,14,16)]
d  <- dist(dcon)
h1 <- hclust(d,method="ward.D")
plot(h1)

library(cluster)
actives<-c(2,8,11,12,13,14,16)
